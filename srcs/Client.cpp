...
{
	ip = inet_ntoa(info.sin_addr); // 127.0.0.1
	port = ft::ft_htons(info.sin_port); //8080
	rBuf = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1)); //설정한 버퍼사이즈
	memset((void *)rBuf, 0, BUFFER_SIZE + 1); // 0 초기화
	...
	chunk.done = false;
	chunk.found = false;
	last_date = ft::getDate(); // 현재 시간
	g_logger.log("new connection from " + ip + ":" + std::to_string(port), LOW); // 출력
}
